...
def f_get_conventional_MeOH_CO2_intensity():
        m1 = math.log((CO2_ROI_Constants_Zero_Heat_Zero_Heat.CO2_emissions_Conv_MeOH_mean**2)/math.sqrt(CO2_ROI_Constants_Zero_Heat_Zero_Heat.CO2_emissions_Conv_MeOH_sd**2+CO2_ROI_Constants_Zero_Heat.CO2_emissions_Conv_MeOH_mean**2))
        sd1= math.sqrt(math.log(CO2_ROI_Constants_Zero_Heat.CO2_emissions_Conv_MeOH_sd**2/(CO2_ROI_Constants_Zero_Heat.CO2_emissions_Conv_MeOH_mean**2)+1))
        conventional_MeOH_CO2_intensity_array_array=np.random.lognormal(m1, sd1, CO2_ROI_Constants_Zero_Heat.number_of_samples)
        return conventional_MeOH_CO2_intensity_array_array
    
def f_get_CCU_MeOH_CO2_intensity_without_H2():
    m1 = math.log((CO2_ROI_Constants_Zero_Heat.CO2_emissions_CCU_MeOH_without_H2_mean**2)/math.sqrt(CO2_ROI_Constants_Zero_Heat.CO2_emissions_CCU_MeOH_without_H2_sd**2+CO2_ROI_Constants_Zero_Heat.CO2_emissions_CCU_MeOH_without_H2_mean**2))
    sd1= math.sqrt(math.log(CO2_ROI_Constants_Zero_Heat.CO2_emissions_CCU_MeOH_without_H2_sd**2/(CO2_ROI_Constants_Zero_Heat.CO2_emissions_CCU_MeOH_without_H2_mean**2)+1))
    CCU_MeOH_CO2_intensity_without_H2_array_array=np.random.lognormal(m1, sd1, CO2_ROI_Constants_Zero_Heat.number_of_samples)
    return CCU_MeOH_CO2_intensity_without_H2_array_array

def f_get_CCU_MeOH_CO2_intensity_with_H2():
    m1 = math.log((CO2_ROI_Constants_Zero_Heat.CO2_emissions_CCU_MeOH_with_H2_mean**2)/math.sqrt(CO2_ROI_Constants_Zero_Heat.CO2_emissions_CCU_MeOH_with_H2_sd**2+CO2_ROI_Constants_Zero_Heat.CO2_emissions_CCU_MeOH_with_H2_mean**2))
    sd1= math.sqrt(math.log(CO2_ROI_Constants_Zero_Heat.CO2_emissions_CCU_MeOH_with_H2_sd**2/(CO2_ROI_Constants_Zero_Heat.CO2_emissions_CCU_MeOH_with_H2_mean**2)+1))
    CCU_MeOH_CO2_intensity_with_H2_array_array=np.random.lognormal(m1, sd1, CO2_ROI_Constants_Zero_Heat.number_of_samples)
    return CCU_MeOH_CO2_intensity_with_H2_array_array

def f_get_CCU_MeOH_CO2_intensity_with_env_opp_cost():
    m1 = math.log((CO2_ROI_Constants_Zero_Heat.CO2_emissions_CCU_MeOH_with_env_opp_cost_mean**2)/math.sqrt(CO2_ROI_Constants_Zero_Heat.CO2_emissions_CCU_MeOH_with_env_opp_cost_sd**2+CO2_ROI_Constants_Zero_Heat.CO2_emissions_CCU_MeOH_with_env_opp_cost_mean**2))
    sd1= math.sqrt(math.log(CO2_ROI_Constants_Zero_Heat.CO2_emissions_CCU_MeOH_with_env_opp_cost_sd**2/(CO2_ROI_Constants_Zero_Heat.CO2_emissions_CCU_MeOH_with_env_opp_cost_mean**2)+1))
    CCU_MeOH_CO2_intensity_with_env_opp_cost_array_array=np.random.lognormal(m1, sd1, CO2_ROI_Constants_Zero_Heat.number_of_samples)
    return CCU_MeOH_CO2_intensity_with_env_opp_cost_array_array

def f_get_CO2_used_per_ton_CCU_MeOH():
    CO2_used_per_ton_CCU_MeOH_array_array=np.random.uniform(CO2_ROI_Constants_Zero_Heat.CO2_used_per_ton_CCU_MeOH_LB,CO2_ROI_Constants_Zero_Heat.CO2_used_per_ton_CCU_MeOH_UB,CO2_ROI_Constants_Zero_Heat.number_of_samples)
...
def f_get_outranking_calculator(super_array):
    ...
    for i in range(len(super_array)):
      ...
      for j in range(0,len(super_array)):
        ...
        outranking_array.append((first_higher_CO2_ROI_than_second/CO2_ROI_Constants_Zero_Heat.number_of_samples))
        
 
    return np.array(outranking_array).reshape(CO2_ROI_Constants_Zero_Heat.number_of_products,CO2_ROI_Constants_Zero_Heat.number_of_products)
...
def f_get_outranking_calculator_for_proof_of_concept(super_array):
    ...
    return np.array(outranking_array).reshape(CO2_ROI_Constants_Zero_Heat.number_of_products_for_proof_of_concept,CO2_ROI_Constants_Zero_Heat.number_of_products_for_proof_of_concept)
...
# to prevent double counting   
    df.loc[(df[CO2_ROI_Constants_Zero_Heat.column_name_Heat_for_CO2_capture_MJ] != 0) | \
           (df[CO2_ROI_Constants_Zero_Heat.column_name_Heat_for_CO2_capture_kWh_Th] != 0)\
              | (df[CO2_ROI_Constants_Zero_Heat.column_name_Electricity_for_CO2_capture_kWh] != 0), CO2_ROI_Constants_Zero_Heat.column_name_CO2_Input]=0
#If the value for the electricity required to generate H2 is mentioned, then set the value of H2 required (kg) to zero
# to prevent double counting          
    df.loc[(df[CO2_ROI_Constants_Zero_Heat.column_name_electricity_for_H2] != 0), CO2_ROI_Constants_Zero_Heat.column_name_H2_Input]=0
...
def f_get_chemical_input_inventory_CO2_intensity_array(number_of_samples,number_of_columns):
    ...
    CO2_energy_penalty=f_get_lognormal_distribution(CO2_ROI_Constants_Zero_Heat.CO2_emissions_CO2_Capture_with_MEA_mean, CO2_ROI_Constants_Zero_Heat.CO2_emissions_CO2_Capture_with_MEA_sd,number_of_samples)
    ...
    CO2_H2_Generation=f_get_lognormal_distribution(CO2_ROI_Constants_Zero_Heat.CO2_emissions_H2_Through_Electrolysis_mean, CO2_ROI_Constants_Zero_Heat.CO2_emissions_H2_Through_Electrolysis_sd,number_of_samples)
    Monte_Carlo_input_inventory_CO2_intensity_matrix[:,1] =CO2_H2_Generation
    
    CO2_water_analyte=f_get_lognormal_distribution(CO2_ROI_Constants_Zero_Heat.CO2_emissions_water_analyte_mean, CO2_ROI_Constants_Zero_Heat.CO2_emissions_water_analyte_sd, number_of_samples)
    Monte_Carlo_input_inventory_CO2_intensity_matrix[:,2] =CO2_water_analyte
    
    CO2_water_cooling=f_get_lognormal_distribution(CO2_ROI_Constants_Zero_Heat.CO2_emissions_water_cooling_mean, CO2_ROI_Constants_Zero_Heat.CO2_emissions_water_cooling_sd, number_of_samples)
    Monte_Carlo_input_inventory_CO2_intensity_matrix[:,3] =CO2_water_cooling
    
    CO2_NaOH=f_get_lognormal_distribution(CO2_ROI_Constants_Zero_Heat.CO2_emissions_NaOH_mean, CO2_ROI_Constants_Zero_Heat.CO2_emissions_NaOH_sd, number_of_samples)
    Monte_Carlo_input_inventory_CO2_intensity_matrix[:,4] =CO2_NaOH
    
    CO2_HCl=f_get_lognormal_distribution(CO2_ROI_Constants_Zero_Heat.CO2_emissions_HCl_mean, CO2_ROI_Constants_Zero_Heat.CO2_emissions_HCl_sd, number_of_samples)
    Monte_Carlo_input_inventory_CO2_intensity_matrix[:,5] =CO2_HCl
    
    CO2_NaCl=f_get_lognormal_distribution(CO2_ROI_Constants_Zero_Heat.CO2_emissions_NaCl_mean, CO2_ROI_Constants_Zero_Heat.CO2_emissions_NaCl_sd, number_of_samples)
    Monte_Carlo_input_inventory_CO2_intensity_matrix[:,6] =CO2_NaCl
    
    Heat_to_electricity_array=f_get_uniform_distribution(CO2_ROI_Constants_Zero_Heat.Heat_to_electricity_LB, CO2_ROI_Constants_Zero_Heat.Heat_to_electricity_UB,number_of_samples)
    US_elec_CO2_intensity_array=f_get_uniform_distribution(CO2_ROI_Constants_Zero_Heat.US_elec_CO2_intensity_LB, CO2_ROI_Constants_Zero_Heat.US_elec_CO2_intensity_UB,number_of_samples)
    ...
    CO2_wind_electricity=f_get_lognormal_distribution(CO2_ROI_Constants_Zero_Heat.CO2_emissions_wind_electricity_mean, CO2_ROI_Constants_Zero_Heat.CO2_emissions_wind_electricity_sd, number_of_samples)
    ...
    CO2_natural_gas_per_kg=f_get_lognormal_distribution(CO2_ROI_Constants_Zero_Heat.CO2_emissions_natural_gas_per_kg_mean, CO2_ROI_Constants_Zero_Heat.CO2_emissions_natural_gas_per_kg_sd,number_of_samples)
    Monte_Carlo_input_inventory_CO2_intensity_matrix[:,12] = CO2_natural_gas_per_kg
    
    CO2_natural_gas_per_m3=f_get_lognormal_distribution(CO2_ROI_Constants_Zero_Heat.CO2_emissions_natural_gas_per_m3_mean, CO2_ROI_Constants_Zero_Heat.CO2_emissions_natural_gas_per_m3_sd, number_of_samples)
    Monte_Carlo_input_inventory_CO2_intensity_matrix[:,13] = CO2_natural_gas_per_m3
    
    CO2_heat_per_MJ=f_get_lognormal_distribution(CO2_ROI_Constants_Zero_Heat.CO2_emissions_heat_per_MJ_mean, CO2_ROI_Constants_Zero_Heat.CO2_emissions_heat_per_MJ_sd, number_of_samples)
    ...
    CO2_steam_per_MJ=f_get_lognormal_distribution(CO2_ROI_Constants_Zero_Heat.CO2_emissions_steam_per_MJ_mean, CO2_ROI_Constants_Zero_Heat.CO2_emissions_steam_per_MJ_sd, number_of_samples)
    Monte_Carlo_input_inventory_CO2_intensity_matrix[:,16] = CO2_steam_per_MJ
    
    CO2_steam_per_kg=f_get_lognormal_distribution(CO2_ROI_Constants_Zero_Heat.CO2_emissions_steam_per_kg_mean, CO2_ROI_Constants_Zero_Heat.CO2_emissions_steam_per_kg_sd, number_of_samples)
    ...
    CO2_MeOH=f_get_lognormal_distribution(CO2_ROI_Constants_Zero_Heat.CO2_emissions_MeOH_conventional_mean, CO2_ROI_Constants_Zero_Heat.CO2_emissions_MeOH_conventional_sd, number_of_samples)
    Monte_Carlo_input_inventory_CO2_intensity_matrix[:,19] = CO2_MeOH
    
    CO2_ammonia=f_get_lognormal_distribution(CO2_ROI_Constants_Zero_Heat.CO2_emissions_ammonia_mean, CO2_ROI_Constants_Zero_Heat.CO2_emissions_ammonia_sd, number_of_samples)
    Monte_Carlo_input_inventory_CO2_intensity_matrix[:,20] = CO2_ammonia
    
    CO2_ethylene_oxide=f_get_lognormal_distribution(CO2_ROI_Constants_Zero_Heat.CO2_emissions_ethylene_oxide_mean, CO2_ROI_Constants_Zero_Heat.CO2_emissions_ethylene_oxide_sd, number_of_samples)
    Monte_Carlo_input_inventory_CO2_intensity_matrix[:,21] = CO2_ethylene_oxide
    
    CO2_platinum=f_get_lognormal_distribution(CO2_ROI_Constants_Zero_Heat.CO2_emissions_platinum_mean, CO2_ROI_Constants_Zero_Heat.CO2_emissions_platinum_sd, number_of_samples)
    Monte_Carlo_input_inventory_CO2_intensity_matrix[:,22] = CO2_platinum
    
    CO2_niobium=f_get_uniform_distribution(CO2_ROI_Constants_Zero_Heat.CO2_emissions_niobium_LB, CO2_ROI_Constants_Zero_Heat.CO2_emissions_niobium_UB,number_of_samples)
    Monte_Carlo_input_inventory_CO2_intensity_matrix[:,23] =CO2_niobium
    
    CO2_aniline=f_get_lognormal_distribution(CO2_ROI_Constants_Zero_Heat.CO2_emissions_aniline_mean, CO2_ROI_Constants_Zero_Heat.CO2_emissions_aniline_sd, number_of_samples)
    Monte_Carlo_input_inventory_CO2_intensity_matrix[:,24] = CO2_aniline
    
    CO2_propylene_oxide=f_get_lognormal_distribution(CO2_ROI_Constants_Zero_Heat.CO2_emissions_propylene_oxide_mean, CO2_ROI_Constants_Zero_Heat.CO2_emissions_propylene_oxide_sd, number_of_samples)
    Monte_Carlo_input_inventory_CO2_intensity_matrix[:,25] = CO2_propylene_oxide
    
    CO2_glycerol=f_get_lognormal_distribution(CO2_ROI_Constants_Zero_Heat.CO2_emissions_glycerol_mean, CO2_ROI_Constants_Zero_Heat.CO2_emissions_glycerol_sd, number_of_samples)
    Monte_Carlo_input_inventory_CO2_intensity_matrix[:,26] = CO2_glycerol
    
    CO2_monopropylene_glycol=f_get_lognormal_distribution(CO2_ROI_Constants_Zero_Heat.CO2_emissions_monopropylene_glycol_mean, CO2_ROI_Constants_Zero_Heat.CO2_emissions_monopropylene_glycol_sd, number_of_samples)
    Monte_Carlo_input_inventory_CO2_intensity_matrix[:,27] = CO2_monopropylene_glycol
    
    CO2_naphtha=f_get_lognormal_distribution(CO2_ROI_Constants_Zero_Heat.CO2_emissions_naphtha_mean, CO2_ROI_Constants_Zero_Heat.CO2_emissions_naphtha_sd, number_of_samples)
    Monte_Carlo_input_inventory_CO2_intensity_matrix[:,28] = CO2_naphtha
    
    CO2_refrigerant=f_get_lognormal_distribution(CO2_ROI_Constants_Zero_Heat.CO2_emissions_refrigerant_mean, CO2_ROI_Constants_Zero_Heat.CO2_emissions_refrigerant_sd, number_of_samples)
    Monte_Carlo_input_inventory_CO2_intensity_matrix[:,29] = CO2_refrigerant
    
    CO2_activated_carbon=f_get_lognormal_distribution(CO2_ROI_Constants_Zero_Heat.CO2_emissions_activated_carbon_mean, CO2_ROI_Constants_Zero_Heat.CO2_emissions_activated_carbon_sd, number_of_samples)
...
def f_get_chemical_input_inventory_CO2_intensity_for_sensitvity_analysis_array(number_of_samples,number_of_columns):
    ...
    CO2_heat_per_kWh=f_get_lognormal_distribution(CO2_ROI_Constants_Zero_Heat.CO2_emissions_heat_per_kWh_mean, CO2_ROI_Constants_Zero_Heat.CO2_emissions_heat_per_kWh_sd, number_of_samples)
...
def f_get_monte_carlo_byproducts_CO2_intensity_matrix(number_of_samples,number_of_columns):
    ...
    CO2_H2_conventional=f_get_uniform_distribution(CO2_ROI_Constants_Zero_Heat.CO2_emissions_H2_conventional_LB, CO2_ROI_Constants_Zero_Heat.CO2_emissions_H2_conventional_UB, number_of_samples)
    Monte_Carlo_conventional_pathway_CO2_intensity_matrix[:,0] = CO2_H2_conventional
    
    CO2_O2_conventional=f_get_lognormal_distribution(CO2_ROI_Constants_Zero_Heat.CO2_emissions_O2_conventional_mean, CO2_ROI_Constants_Zero_Heat.CO2_emissions_O2_conventional_sd, number_of_samples)
    Monte_Carlo_conventional_pathway_CO2_intensity_matrix[:,1] = CO2_O2_conventional
    
    CO2_steam_kg_conventional=f_get_lognormal_distribution(CO2_ROI_Constants_Zero_Heat.CO2_emissions_steam_per_kg_mean, CO2_ROI_Constants_Zero_Heat.CO2_emissions_steam_per_kg_sd, number_of_samples)
    Monte_Carlo_conventional_pathway_CO2_intensity_matrix[:,2] = CO2_steam_kg_conventional
    
    CO2_steam_MJ_conventional=f_get_lognormal_distribution(CO2_ROI_Constants_Zero_Heat.CO2_emissions_steam_per_MJ_mean, CO2_ROI_Constants_Zero_Heat.CO2_emissions_steam_per_MJ_sd, number_of_samples)
    ...
    CO2_heat_MJ_conventional=f_get_lognormal_distribution(CO2_ROI_Constants_Zero_Heat.CO2_emissions_heat_per_MJ_mean, CO2_ROI_Constants_Zero_Heat.CO2_emissions_heat_per_MJ_sd, number_of_samples)
    ...
    CO2_sodium_formate_conventional=f_get_lognormal_distribution(CO2_ROI_Constants_Zero_Heat.CO2_emissions_sodium_formate_conventional_mean, CO2_ROI_Constants_Zero_Heat.CO2_emissions_sodium_formate_conventional_sd, number_of_samples)
    Monte_Carlo_conventional_pathway_CO2_intensity_matrix[:,7] = CO2_sodium_formate_conventional
    
    CO2_sodium_carbonate_conventional=f_get_lognormal_distribution(CO2_ROI_Constants_Zero_Heat.CO2_emissions_soda_ash_conventional_mean, CO2_ROI_Constants_Zero_Heat.CO2_emissions_soda_ash_conventional_sd, number_of_samples)
    Monte_Carlo_conventional_pathway_CO2_intensity_matrix[:,8] = CO2_sodium_carbonate_conventional
    
    CO2_sodium_sulfate_conventional=f_get_lognormal_distribution(CO2_ROI_Constants_Zero_Heat.CO2_emissions_sodium_sulfate_conventional_mean, CO2_ROI_Constants_Zero_Heat.CO2_emissions_sodium_sulfate_conventional_sd, number_of_samples)
    Monte_Carlo_conventional_pathway_CO2_intensity_matrix[:,9] = CO2_sodium_sulfate_conventional
    
    CO2_ethylene_glycol_conventional=f_get_lognormal_distribution(CO2_ROI_Constants_Zero_Heat.CO2_emissions_ethylene_glycol_conventional_mean, CO2_ROI_Constants_Zero_Heat.CO2_emissions_ethylene_glycol_conventional_sd, number_of_samples)
    Monte_Carlo_conventional_pathway_CO2_intensity_matrix[:,10] = CO2_ethylene_glycol_conventional
    
    CO2_ethylene_carbonate_conventional=f_get_lognormal_distribution(CO2_ROI_Constants_Zero_Heat.CO2_emissions_ethylene_carbonate_conventional_mean, CO2_ROI_Constants_Zero_Heat.CO2_emissions_ethylene_carbonate_conventional_sd, number_of_samples)
    Monte_Carlo_conventional_pathway_CO2_intensity_matrix[:,11] = CO2_ethylene_carbonate_conventional
    
    CO2_naphtha_conventional=f_get_lognormal_distribution(CO2_ROI_Constants_Zero_Heat.CO2_emissions_naphtha_conventional_mean, CO2_ROI_Constants_Zero_Heat.CO2_emissions_naphtha_conventional_sd, number_of_samples)
    Monte_Carlo_conventional_pathway_CO2_intensity_matrix[:,12] = CO2_naphtha_conventional
    
    CO2_water_conventional=f_get_lognormal_distribution(CO2_ROI_Constants_Zero_Heat.CO2_emissions_water_conventional_mean, CO2_ROI_Constants_Zero_Heat.CO2_emissions_water_conventional_sd, number_of_samples)
...
def f_get_monte_carlo_waste_CO2_intensity_matrix(number_of_samples,number_of_columns):
    ...
    CO2_waste_water_emissions=f_get_lognormal_distribution(CO2_ROI_Constants_Zero_Heat.CO2_emissions_waste_water_mean,CO2_ROI_Constants_Zero_Heat.CO2_emissions_waste_water_sd, number_of_samples)
    ...
    CO2_waste_zeolite_emissions=f_get_lognormal_distribution(CO2_ROI_Constants_Zero_Heat.CO2_emissions_waste_zeolite_mean,CO2_ROI_Constants_Zero_Heat.CO2_emissions_waste_zeolite_sd, number_of_samples)
...
def f_get_monte_carlo_chemical_conventional_pathway_CO2_intensity_matrix(number_of_samples,number_datasets_formic_acid_catalytic_reduction_CO2_with_H2, mean, sd, distribution_type):
    ...
    if(distribution_type==CO2_ROI_Constants_Zero_Heat.lognormal_distribution):
    ...
    elif(distribution_type==CO2_ROI_Constants_Zero_Heat.uniform_distribution):
...
def f_get_climate_ROI_for_chemical_list(CCU_chemical_name,\
    ...
    excel_tab_df = pd.read_excel(CO2_ROI_Constants_Zero_Heat.excel_file, sheet_name=CO2_ROI_Constants_Zero_Heat.sheet_name_chemicals)
    
    #filter the rows for the specific chemical and production route
    CCU_input_inventory_un_harmonized_df = \
    excel_tab_df[(excel_tab_df[CO2_ROI_Constants_Zero_Heat.column_name_CCU_Product]==CCU_chemical_name) & \
                 (excel_tab_df[CO2_ROI_Constants_Zero_Heat.column_name_Production_Route]==CCU_chemical_production_route)]
    ...
    CO2_Utilized_list=CCU_input_inventory_un_harmonized_df[CO2_ROI_Constants_Zero_Heat.column_name_CO2_Input].values.tolist()
    ...
    (CCU_input_byproduct_waste_inventory_ALL_chemcials_harmonized_df[CO2_ROI_Constants_Zero_Heat.column_name_CCU_Product]==CCU_chemical_name) \
    & \
    (CCU_input_byproduct_waste_inventory_ALL_chemcials_harmonized_df[CO2_ROI_Constants_Zero_Heat.column_name_Production_Route]==CCU_chemical_production_route)]
    ...
    f_get_chemical_input_inventory_CO2_intensity_array(int(round(CO2_ROI_Constants_Zero_Heat.number_of_samples/number_of_datasets_for_CCU_chemical)),CO2_ROI_Constants_Zero_Heat.number_of_input_parameters+1)
    
    #Get the inventory items, which were used to produce the chemical through the CCU pathway
    CCU_input_inventory_array=np.zeros((number_of_datasets_for_CCU_chemical,CO2_ROI_Constants_Zero_Heat.number_of_input_parameters+1))
    CCU_input_inventory_array[:,0:CO2_ROI_Constants_Zero_Heat.number_of_input_parameters]=np.array(CCU_input_byproduct_waste_inventory_for_specific_chemical_harmonized_df.loc[:,CO2_ROI_Constants_Zero_Heat.column_name_CO2_Input:CO2_ROI_Constants_Zero_Heat.column_name_Activated_Carbon_Input].values.tolist())
    #Add an extra column to the inventory array account for the 10% of the CO2 emissions not captured by the MEA system
    CCU_input_inventory_array[:,CO2_ROI_Constants_Zero_Heat.number_of_input_parameters]=np.array(CCU_input_inventory_un_harmonized_df[CO2_ROI_Constants_Zero_Heat.column_name_CO2_Input].values.tolist())/9
    ...
    CCU_byproducts_CO2_intensity_array=f_get_monte_carlo_byproducts_CO2_intensity_matrix(int(round(CO2_ROI_Constants_Zero_Heat.number_of_samples/number_of_datasets_for_CCU_chemical)),CO2_ROI_Constants_Zero_Heat.number_of_byproducts)
    #Get the values of the byproducts that were produced along with the chemical
    CCU_byproduct_inventory_array=np.zeros((number_of_datasets_for_CCU_chemical,CO2_ROI_Constants_Zero_Heat.number_of_byproducts))
    CCU_byproduct_inventory_array[:,0:CO2_ROI_Constants_Zero_Heat.number_of_byproducts]=np.array(CCU_input_byproduct_waste_inventory_for_specific_chemical_harmonized_df.loc[:,CO2_ROI_Constants_Zero_Heat.column_name_H2_byproduct_output:CO2_ROI_Constants_Zero_Heat.column_name_H2O_byproduct_output].values.tolist())
    ...
    CCU_waste_CO2_intensity_array=f_get_monte_carlo_waste_CO2_intensity_matrix(int(round(CO2_ROI_Constants_Zero_Heat.number_of_samples/number_of_datasets_for_CCU_chemical)),CO2_ROI_Constants_Zero_Heat.number_of_waste)
    
    #Get the values of the wastes that were produced along with the chemical
    CCU_waste_inventory_array=np.zeros((number_of_datasets_for_CCU_chemical,CO2_ROI_Constants_Zero_Heat.number_of_waste))
    
    CCU_waste_inventory_array[:,0:CO2_ROI_Constants_Zero_Heat.number_of_waste]=\
    np.array(CCU_input_byproduct_waste_inventory_for_specific_chemical_harmonized_df.loc\
    [:,CO2_ROI_Constants_Zero_Heat.column_name_CO2_emissions_waste:CO2_ROI_Constants_Zero_Heat.column_name_CO_emissions_waste].values.tolist())
    ...
    (int(round(CO2_ROI_Constants_Zero_Heat.number_of_samples/number_of_datasets_for_CCU_chemical)),\
    ...
    H2_inventory_array=np.array(CCU_input_byproduct_waste_inventory_for_specific_chemical_harmonized_df\
                                [[CO2_ROI_Constants_Zero_Heat.column_name_H2_Input,\
                                  CO2_ROI_Constants_Zero_Heat.column_name_electricity_for_H2]].values.tolist())
    
    #calculate the delta in the CO2 intensity between the US grid and wind electricity
    wind_elec_CO2_intensity_array=\
    np.array(f_get_lognormal_distribution(CO2_ROI_Constants_Zero_Heat.CO2_emissions_wind_electricity_mean, \
    CO2_ROI_Constants_Zero_Heat.CO2_emissions_wind_electricity_sd, int(round(CO2_ROI_Constants_Zero_Heat.number_of_samples/number_of_datasets_for_CCU_chemical))))
    
    US_elec_CO2_intensity_array=np.array(f_get_uniform_distribution(CO2_ROI_Constants_Zero_Heat.US_elec_CO2_intensity_LB, \
    CO2_ROI_Constants_Zero_Heat.US_elec_CO2_intensity_UB,int(round(CO2_ROI_Constants_Zero_Heat.number_of_samples/number_of_datasets_for_CCU_chemical))))
    ...
    EOC_array=np.zeros((2,int(round(CO2_ROI_Constants_Zero_Heat.number_of_samples/number_of_datasets_for_CCU_chemical))))
    EOC_array[0:,]=delta_elec_CO2_intensity_array*CO2_ROI_Constants_Zero_Heat.electricity_H2_electrolysis_kWh
...
def f_get_concrete_input_inventory_CO2_intensity_matrix(number_of_samples,number_of_columns):
    ...
    CO2_cement=f_get_lognormal_distribution(CO2_ROI_Constants_Zero_Heat.cement_CO2_intensity_mean, CO2_ROI_Constants_Zero_Heat.cement_CO2_intensity_sd,number_of_samples)
    concrete_input_inventory_CO2_intensity_matrix[:,0] = CO2_cement
  
 
    CO2_coarse_agg=f_get_lognormal_distribution(CO2_ROI_Constants_Zero_Heat.coarse_agg_CO2_intensity_mean, CO2_ROI_Constants_Zero_Heat.coarse_agg_CO2_intensity_sd,number_of_samples)
    concrete_input_inventory_CO2_intensity_matrix[:,1] = CO2_coarse_agg
    
    CO2_fine_agg=f_get_lognormal_distribution(CO2_ROI_Constants_Zero_Heat.fine_agg_CO2_intensity_mean, CO2_ROI_Constants_Zero_Heat.fine_agg_CO2_intensity_sd,number_of_samples)
    concrete_input_inventory_CO2_intensity_matrix[:,2] = CO2_fine_agg
    
    CO2_water=f_get_lognormal_distribution(CO2_ROI_Constants_Zero_Heat.water_CO2_intensity_mean, CO2_ROI_Constants_Zero_Heat.water_CO2_intensity_sd,number_of_samples)
    concrete_input_inventory_CO2_intensity_matrix[:,3] = CO2_water

    CO2_Slag=7.7*f_get_lognormal_distribution(CO2_ROI_Constants_Zero_Heat.Pig_Iron_CO2_intensity_mean, CO2_ROI_Constants_Zero_Heat.Pig_Iron_CO2_intensity_sd, number_of_samples)
    concrete_input_inventory_CO2_intensity_matrix[:,4] = CO2_Slag

    CO2_fly_ash=22.7*f_get_lognormal_distribution(CO2_ROI_Constants_Zero_Heat.Coal_Elec_CO2_intensity_mean, CO2_ROI_Constants_Zero_Heat.Coal_Elec_CO2_intensity_sd, number_of_samples)
    ...
    CO2_steam_curing_CO2_emissions=f_get_uniform_distribution(CO2_ROI_Constants_Zero_Heat.steam_curing_LB, CO2_ROI_Constants_Zero_Heat.steam_curing_UB,number_of_samples)
    concrete_input_inventory_CO2_intensity_matrix[:,7] = CO2_steam_curing_CO2_emissions
    
    Power_Required_CO2_Curing=CO2_ROI_Constants_Zero_Heat.Power_Required_CO2_Curing
    CCU_CO2_Curing_CO2_intensity_array=f_get_uniform_distribution(CO2_ROI_Constants_Zero_Heat.CCU_CO2_Curing_Elec_CO2_intensity_LB,CO2_ROI_Constants_Zero_Heat.CCU_CO2_Curing_Elec_CO2_intensity_UB,number_of_samples)
    concrete_input_inventory_CO2_intensity_matrix[:,8] = Power_Required_CO2_Curing*CCU_CO2_Curing_CO2_intensity_array
    
    CO2_CO2_Injection=f_get_uniform_distribution(CO2_ROI_Constants_Zero_Heat.CCU_Injection_Elec_CO2_Intensity_LB,CO2_ROI_Constants_Zero_Heat.CCU_Injection_Elec_CO2_Intensity_UB,number_of_samples)
    concrete_input_inventory_CO2_intensity_matrix[:,9] = CO2_CO2_Injection
    
    CO2_CO2_Vaporization=f_get_uniform_distribution(CO2_ROI_Constants_Zero_Heat.CCU_Vaporization_Elec_CO2_Intensity_LB,CO2_ROI_Constants_Zero_Heat.CCU_Vaporization_Elec_CO2_Intensity_UB,number_of_samples)
...
def f_get_climate_ROI_for_concrete_list(excel_sheet_name,\
    ...
    concrete_un_harmonized_df = pd.read_excel(CO2_ROI_Constants_Zero_Heat.excel_file, excel_sheet_name)
    ...
    concrete_harmonized_df.loc[concrete_harmonized_df[CO2_ROI_Constants_Zero_Heat.column_name_category].isin([dataset_category])]
    ...
    concrete_input_inventory_CO2_intensity_array=f_get_concrete_input_inventory_CO2_intensity_matrix(\
                                                    int(round(CO2_ROI_Constants_Zero_Heat.number_of_samples/number_of_datasets_for_CCU_concrete)),\
                                                    CO2_ROI_Constants_Zero_Heat.number_of_input_parameters_concrete+3)
    ...
    CCU_concrete_input_inventory_array=np.zeros((number_of_datasets_for_CCU_concrete,\
                                                 CO2_ROI_Constants_Zero_Heat.number_of_input_parameters_concrete+3))
    CCU_concrete_input_inventory_array[:,0:CO2_ROI_Constants_Zero_Heat.number_of_input_parameters_concrete]=\
    ...
    conventional_concrete_input_inventory_array[:,0:CO2_ROI_Constants_Zero_Heat.number_of_input_parameters_concrete]=\
   ...
   #(2) CO2 vaporization and (3)10% not captured at the power plant
    CCU_concrete_input_inventory_array[:,CO2_ROI_Constants_Zero_Heat.number_of_input_parameters_concrete:\
                                       CO2_ROI_Constants_Zero_Heat.number_of_input_parameters_concrete+3]=\
                                       np.array(concrete_input_inventory_harmonized_df[\
                                      [CO2_ROI_Constants_Zero_Heat.column_name_CCU_CO2_Utilized_Duplicate1,\
                                       CO2_ROI_Constants_Zero_Heat.column_name_CCU_CO2_Utilized_Duplicate2,\
                                       CO2_ROI_Constants_Zero_Heat.column_name_CCU_CO2_Utilized_Duplicate3]].values.tolist())
    
    #Append the last three columns which are required to account for the CO2 emissions from (1)CO2 injection
    #(2) CO2 vaporization and (3)10% not captured at the power plant
    conventional_concrete_input_inventory_array[:,CO2_ROI_Constants_Zero_Heat.number_of_input_parameters_concrete:\
                                       CO2_ROI_Constants_Zero_Heat.number_of_input_parameters_concrete+3]=\
                                       np.array(concrete_input_inventory_harmonized_df[\
                                      [CO2_ROI_Constants_Zero_Heat.column_name_conventional_CO2_Utilized_Duplicate1,\
                                       CO2_ROI_Constants_Zero_Heat.column_name_conventional_CO2_Utilized_Duplicate2,\
                                       CO2_ROI_Constants_Zero_Heat.column_name_conventional_CO2_Utilized_Duplicate3]].values.tolist())
...
def f_get_delta_indices_chemical(CCU_chemical_name,CCU_chemical_production_route,\
    ...
    #The CO2 values will be used to calculate the climate ROI 
    CO2_Utilized_array=\
        np.transpose(np.array([CCU_input_inventory_un_harmonized_df[CO2_ROI_Constants_Zero_Heat.column_name_CO2_Input].values.tolist()]))
    ...
    CCU_input_byproduct_waste_inventory_for_specific_chemical_harmonized_df = \
        CCU_input_byproduct_waste_inventory_ALL_chemcials_harmonized_df[\
            (CCU_input_byproduct_waste_inventory_ALL_chemcials_harmonized_df[CO2_ROI_Constants_Zero_Heat.column_name_CCU_Product]\
             ==CCU_chemical_name) \
            & \
            (CCU_input_byproduct_waste_inventory_ALL_chemcials_harmonized_df[CO2_ROI_Constants_Zero_Heat.column_name_Production_Route]\
             ==CCU_chemical_production_route)]
...
#    tt= (int(round(CO2_ROI_Constants_Zero_Heat.number_of_samples/number_of_datasets_for_CCU_chemical)))
...
#    CO2_Utilized_array = np.repeat(CO2_Utilized_array,repeats=[tt],axis=0)
    ...
    (CO2_ROI_Constants_Zero_Heat.number_of_samples,CO2_ROI_Constants_Zero_Heat.number_of_input_parameters+1)
...
#    print('CCU_input_inventory_CO2_intensity_array:',CCU_input_inventory_CO2_intensity_array)
    ...
    #Get the inventory items, which were used to produce the chemical through the CCU pathway
    CCU_input_inventory_array=np.zeros((number_of_datasets_for_CCU_chemical,CO2_ROI_Constants_Zero_Heat.number_of_input_parameters+1))
    CCU_input_inventory_array[:,0:CO2_ROI_Constants_Zero_Heat.number_of_input_parameters]=\
    np.array(CCU_input_byproduct_waste_inventory_for_specific_chemical_harmonized_df.loc[:,CO2_ROI_Constants_Zero_Heat.column_name_CO2_Input:CO2_ROI_Constants_Zero_Heat.column_name_Activated_Carbon_Input].values.tolist())
    ...
    conventional_pathway_CO2_intensity_array=\
        ...
        CO2_ROI_Constants_Zero_Heat.number_of_samples,\
    ...
    CCU_waste_CO2_intensity_array=\
            f_get_monte_carlo_waste_CO2_intensity_matrix(CO2_ROI_Constants_Zero_Heat.number_of_samples,CO2_ROI_Constants_Zero_Heat.number_of_waste)
            
    #calculate the delta in the CO2 intensity between the US grid and wind electricity
    wind_elec_CO2_intensity_array=\
        np.transpose(np.array([f_get_lognormal_distribution(CO2_ROI_Constants_Zero_Heat.CO2_emissions_wind_electricity_mean, \
        CO2_ROI_Constants_Zero_Heat.CO2_emissions_wind_electricity_sd,\
        CO2_ROI_Constants_Zero_Heat.number_of_samples)]))
        
    US_elec_CO2_intensity_array=\
        np.transpose(np.array([f_get_uniform_distribution(CO2_ROI_Constants_Zero_Heat.US_elec_CO2_intensity_LB, \
        CO2_ROI_Constants_Zero_Heat.US_elec_CO2_intensity_UB,CO2_ROI_Constants_Zero_Heat.number_of_samples)]))
...
#        print('CCU_input_inventory_CO2_emissions_array.shape:',CCU_input_inventory_CO2_emissions_array.shape)
        ...
        CCU_byproducts_CO2_intensity_array\
            =f_get_monte_carlo_byproducts_CO2_intensity_matrix(CO2_ROI_Constants_Zero_Heat.number_of_samples,\
             CO2_ROI_Constants_Zero_Heat.number_of_byproducts)
       
    
        #Get the values of the byproducts that were produced along with the chemical
        CCU_byproduct_inventory_array=np.zeros((number_of_datasets_for_CCU_chemical,\
                                                CO2_ROI_Constants_Zero_Heat.number_of_byproducts))
        CCU_byproduct_inventory_array[:,0:CO2_ROI_Constants_Zero_Heat.number_of_byproducts]=\
                 np.array(CCU_input_byproduct_waste_inventory_for_specific_chemical_harmonized_df.loc\
                 [:,CO2_ROI_Constants_Zero_Heat.column_name_H2_byproduct_output:CO2_ROI_Constants_Zero_Heat.column_name_H2O_byproduct_output].values.tolist())
...
#        print('np.array([H2_inventory_array[0]])', np.array([H2_inventory_array[i]]))
    ...
    #    H2_inventory_array = np.repeat(H2_inventory_array,repeats=[tt],axis=0)
        ...
        #The first column in EOC array will be multipied by the H2 values (kg) from the H2 inventory array
        EOC_array=np.zeros((CO2_ROI_Constants_Zero_Heat.number_of_samples,2))
          
        EOC_array[:,0]=(delta_elec_CO2_intensity_array*CO2_ROI_Constants_Zero_Heat.electricity_H2_electrolysis_kWh)[:,0]
        #The second column will be multipied by the electrcity required for H2 generation (kWh) from the H2 inventory array
...
#        print('CO2_EOC_array', CO2_EOC_array)
        ...
        input_array= \
            np.hstack((\
                       CCU_input_inventory_CO2_emissions_array[:,0:CO2_ROI_Constants_Zero_Heat.number_of_input_parameters],\
                       CCU_byproduct_CO2_emissions_array,\
...
#        print('np.array([CO2_Utilized_array[0]]).shape    :', np.array([CO2_Utilized_array[i]]))
        ...
        if(EOC_Included==CO2_ROI_Constants_Zero_Heat.EOC_No): 
            output_array= \
                    np.hstack((\
...
#        print('output_array1    :')
    ...
    #                'conventional emissions',\
                    CO2_ROI_Constants_Zero_Heat.column_name_CO2_Input,\
                    CO2_ROI_Constants_Zero_Heat.column_name_H2_Input,\
                    CO2_ROI_Constants_Zero_Heat.column_name_H2O_as_analyte_Input,\
                    CO2_ROI_Constants_Zero_Heat.column_name_H2O_for_cooling_Input,\
                    CO2_ROI_Constants_Zero_Heat.column_name_NaOH_Input,\
                    CO2_ROI_Constants_Zero_Heat.column_name_HCl_Input,\
                    CO2_ROI_Constants_Zero_Heat.column_name_NaCl_Input,\
                    CO2_ROI_Constants_Zero_Heat.column_name_Heat_for_CO2_capture_MJ,\
                    CO2_ROI_Constants_Zero_Heat.column_name_Heat_for_CO2_capture_kWh_Th,\
                    CO2_ROI_Constants_Zero_Heat.column_name_Electricity_for_CO2_capture_kWh,\
                    CO2_ROI_Constants_Zero_Heat.column_name_electricity_for_H2,\
                    CO2_ROI_Constants_Zero_Heat.column_name_Electricity_for_main_reaction_kWh_Input,\
                    CO2_ROI_Constants_Zero_Heat.column_name_Natural_Gas_kg_Input,\
                    CO2_ROI_Constants_Zero_Heat.column_name_Natural_Gas_m3_Input,\
                    CO2_ROI_Constants_Zero_Heat.column_name_Heat_kWh_Input,\
                    CO2_ROI_Constants_Zero_Heat.column_name_Heat_MJ_Input,\
                    CO2_ROI_Constants_Zero_Heat.column_name_Steam_MJ_Input,\
                    CO2_ROI_Constants_Zero_Heat.column_name_Steam_kg_Input,\
                    CO2_ROI_Constants_Zero_Heat.column_name_Steam_kWh_Input,\
                    CO2_ROI_Constants_Zero_Heat.column_name_Methanol_Input,\
                    CO2_ROI_Constants_Zero_Heat.column_name_Ammonia_Input,\
                    CO2_ROI_Constants_Zero_Heat.column_name_Ethylene_Oxide_Input,\
                    CO2_ROI_Constants_Zero_Heat.column_name_Platinum_Input,\
                    CO2_ROI_Constants_Zero_Heat.column_name_Niobium_Input,\
                    CO2_ROI_Constants_Zero_Heat.column_name_Aniline_Input,\
                    CO2_ROI_Constants_Zero_Heat.column_name_Propylene_Input,\
                    CO2_ROI_Constants_Zero_Heat.column_name_Glycerol_Input,\
                    CO2_ROI_Constants_Zero_Heat.column_name_Mono_Propylene_Glycol_Input,\
                    CO2_ROI_Constants_Zero_Heat.column_name_Naptha_Burned_Input,\
                    CO2_ROI_Constants_Zero_Heat.column_name_Refrigerant_Input,\
                    CO2_ROI_Constants_Zero_Heat.column_name_Activated_Carbon_Input,\
                    CO2_ROI_Constants_Zero_Heat.column_name_H2_byproduct_output,\
                    CO2_ROI_Constants_Zero_Heat.column_name_O2_byproduct,\
                    CO2_ROI_Constants_Zero_Heat.column_name_Steam_kg_Byproduct,\
                    CO2_ROI_Constants_Zero_Heat.column_name_Steam_MJ_Byproduct,\
                    CO2_ROI_Constants_Zero_Heat.column_name_Steam_kWth_Byproduct,\
                    CO2_ROI_Constants_Zero_Heat.column_name_Heat_MJ_Byproduct,\
                    CO2_ROI_Constants_Zero_Heat.column_name_Heat_kWh_Byproduct ,\
                    CO2_ROI_Constants_Zero_Heat.column_name_HCOONa_Byproduct ,\
                    CO2_ROI_Constants_Zero_Heat.column_name_Na2CO3_Byproduct,\
                    CO2_ROI_Constants_Zero_Heat.column_name_Na2SO4_Byproduct,\
                    CO2_ROI_Constants_Zero_Heat.column_name_Ethylene_Glycol_Byproduct,\
                    CO2_ROI_Constants_Zero_Heat.column_name_Ethylene_Carbonate_Byproduct,\
                    CO2_ROI_Constants_Zero_Heat.column_name_Naphtha_Byproduct,\
                    CO2_ROI_Constants_Zero_Heat.column_name_H2O_byproduct_output,\
                    CO2_ROI_Constants_Zero_Heat.column_name_CO2_emissions_waste,\
                    CO2_ROI_Constants_Zero_Heat.column_name_Waste_water_waste,\
                    CO2_ROI_Constants_Zero_Heat.column_name_Flue_gas_waste,\
                    CO2_ROI_Constants_Zero_Heat.column_name_zeolite_inert_landfill_waste,\
                    CO2_ROI_Constants_Zero_Heat.column_name_Methanol_waste,\
                    CO2_ROI_Constants_Zero_Heat.column_name_O2_waste,\
                    CO2_ROI_Constants_Zero_Heat.column_name_N2_waste,\
                    CO2_ROI_Constants_Zero_Heat.column_name_H2O_waste,\
                    CO2_ROI_Constants_Zero_Heat.column_name_Ammonia_waste,\
                    CO2_ROI_Constants_Zero_Heat.column_name_Acetaldehyde_waste,\
                    CO2_ROI_Constants_Zero_Heat.column_name_Aniline_waste,\
                    CO2_ROI_Constants_Zero_Heat.column_name_CO_emissions_waste\
                    ],
